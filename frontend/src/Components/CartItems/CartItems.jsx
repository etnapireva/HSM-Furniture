...
import { cartKey, imageUrl } from "../../config";
...
export default function CartItems() {
  ...
  const handleCheckout = async () => {
    ...
    const orders = products
      .filter((p) => cartItems[cartKey(p)] > 0)
      .map((p) => ({
        product_id:  cartKey(p),
        quantity:    cartItems[cartKey(p)],
        total_price: p.price * cartItems[cartKey(p)],
        customer,
      }));
  ...
  const handleCardPay = async () => {
    ...
    const items = products
      .filter((p) => (cartItems[cartKey(p)] || 0) > 0)
      .map((p) => ({
        product_id: cartKey(p),
        name: p.name,
        amount: p.price,
